...
def get_days_between_transactions(df: pd.DataFrame, filters: dict) -> TableData:
    df = df.sort_values(by=["branch_admin_id", "customer_id", "date"])
    df["days_since"] = df.groupby(["branch_admin_id", "customer_id"])["date"].diff().dt.days

    return TableData(
        metric=f"Days Between Transactions per Customer{get_filter_suffix(filters)}",
        data=df[["branch_admin_id", "customer_id", "date", "days_since"]].to_dict(orient="records")
    )


def get_transaction_outliers(df: pd.DataFrame, filters: dict) -> TableData:
    grouped = (
        df.groupby(["branch_admin_id", "customer_id"])["amount"].sum()
        .reset_index()
...
def get_top_customers(df: pd.DataFrame, mode: str, limit: int, filters: dict) -> TableData:
    if mode == "amount":
        grouped = (
            df.groupby(["branch_admin_id", "customer_id"])["amount"].sum()
            .reset_index()
            .sort_values(by=["branch_admin_id", "amount"], ascending=[True, False])
            .groupby("branch_admin_id")
            .head(limit)
        )
        base_metric = f"Top {limit} Customers by Amount"
    else:  # mode == "count"
        grouped = (
            df.groupby(["branch_admin_id", "customer_id"])["amount"].count()
            .reset_index()
            .rename(columns={"amount": "transaction_count"})
            .sort_values(by=["branch_admin_id", "transaction_count"], ascending=[True, False])
            .groupby("branch_admin_id")
            .head(limit)
        )
...
def apply_branch_admin_date_filters(df, year=None, month=None, week=None, day=None, range_days=None, start_date=None, end_date=None):
