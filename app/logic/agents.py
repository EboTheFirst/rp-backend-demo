...
def get_days_between_transactions(df: pd.DataFrame, filters: dict) -> TableData:
    df = df.sort_values(by=["agent_id", "customer_id", "date"])
    df["days_since"] = df.groupby(["agent_id", "customer_id"])["date"].diff().dt.days

    return TableData(
        metric=f"Days Between Transactions per Customer{get_filter_suffix(filters)}",
        data=df[["agent_id", "customer_id", "date", "days_since"]].to_dict(orient="records")
    )


def get_transaction_outliers(df: pd.DataFrame, filters: dict) -> TableData:
    grouped = (
        df.groupby(["agent_id", "customer_id"])["amount"].sum()
        .reset_index()
...
def get_transaction_outliers_merchants(df: pd.DataFrame, filters: dict) -> TableData:
    grouped = (
        df.groupby(["agent_id", "merchant_id"])["amount"].sum()
        .reset_index()
        .sort_values(by="amount", ascending=False)
    )
    ...
    return TableData(
        metric=f"Merchant Transaction Outliers (±1 STD){get_filter_suffix(filters)}",
        data=outliers.to_dict(orient="records")
    )
...
def get_merchant_segmentation(df: pd.DataFrame, filters: dict) -> TableData:
    merchant_sale = (
        df.groupby("merchant_id")["amount"].sum().reset_index()
        .sort_values(by="amount", ascending=False)
    )

    high_value = merchant_sale[merchant_sale["amount"] > 800]
    mid_value = merchant_sale[(merchant_sale["amount"] <= 800) & (merchant_sale["amount"] > 500)]
    low_value = merchant_sale[merchant_sale["amount"] <= 500]

    metric_label = f"Merchant Segmentation by Total Sales{get_filter_suffix(filters)}"
...
def get_top_merchants(df: pd.DataFrame, mode: str, limit: int, filters: dict) -> TableData:
    if mode == "amount":
        grouped = (
            df.groupby(["agent_id", "merchant_id"])["amount"].sum()
            .reset_index()
            .sort_values(by=["agent_id", "amount"], ascending=[True, False])
            .groupby("agent_id")
            .head(limit)
        )
        base_metric = f"Top {limit} Merchants by Amount"
    else:  # mode == "count"
        grouped = (
            df.groupby(["agent_id", "merchant_id"])["amount"].count()
            .reset_index()
            .rename(columns={"amount": "transaction_count"})
            .sort_values(by=["agent_id", "transaction_count"], ascending=[True, False])
            .groupby("agent_id")
            .head(limit)
        )
        base_metric = f"Top {limit} Merchants by Transaction Count"
...
def get_top_customers(df: pd.DataFrame, mode: str, limit: int, filters: dict) -> TableData:
    if mode == "amount":
        ...
        grouped = (
            df.groupby(["agent_id", "customer_id"])["amount"].count()
            .reset_index()
            .rename(columns={"amount": "transaction_count"})
            .sort_values(by=["agent_id", "transaction_count"], ascending=[True, False])
            .groupby("agent_id")
...
def get_transaction_volume_per_merchant(df: pd.DataFrame, granularity: str, filters: dict = None) -> dict:
    group_cols, label_fmt = get_grouping_and_label_fn(granularity)
    group_cols = ["merchant_id"] + group_cols  # include merchant_id
    ...
    result = {}
    for merchant_id, merchant_df in grouped.groupby("merchant_id"):
        result[str(merchant_id)] = GraphData(
            metric=f"{granularity.capitalize()} Transaction Volume{suffix}",
            data=GraphPoints(
                labels=merchant_df["label"].tolist(),
                values=merchant_df["amount"].round(2).tolist()
            )
        )
...
def get_transaction_count_per_merchant(df: pd.DataFrame, granularity: str, filters: dict = None) -> dict:
    group_cols, label_fmt = get_grouping_and_label_fn(granularity)
    group_cols = ["merchant_id"] + group_cols  # include merchant_id in grouping
    ...
    for merchant_id, merchant_df in grouped.groupby("merchant_id"):
        result[str(merchant_id)] = GraphData(
            ...
            data=GraphPoints(
                labels=merchant_df["label"].tolist(),
                values=merchant_df["amount"].tolist()
            )
        )
