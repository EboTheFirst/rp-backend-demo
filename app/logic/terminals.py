...
def get_days_between_transactions(df: pd.DataFrame, filters: dict) -> TableData:
    df = df.sort_values(by=["terminal_id", "customer_id", "date"])
    df["days_since"] = df.groupby(["terminal_id", "customer_id"])["date"].diff().dt.days

    return TableData(
        metric=f"Days Between Transactions per Customer{get_filter_suffix(filters)}",
        data=df[["terminal_id", "customer_id", "date", "days_since"]].to_dict(orient="records")
    )


def get_transaction_outliers(df: pd.DataFrame, filters: dict) -> TableData:
    grouped = (
        df.groupby(["terminal_id", "customer_id"])["amount"].sum()
        .reset_index()
...
def get_top_customers(df: pd.DataFrame, mode: str, limit: int, filters: dict) -> TableData:
    if mode == "amount":
        grouped = (
            df.groupby(["terminal_id", "customer_id"])["amount"].sum()
            .reset_index()
            .sort_values(by=["terminal_id", "amount"], ascending=[True, False])
            .groupby("terminal_id")
            .head(limit)
        )
        ...
        grouped = (
            df.groupby(["terminal_id", "customer_id"])["amount"].count()
            .reset_index()
            .rename(columns={"amount": "transaction_count"})
            .sort_values(by=["terminal_id", "transaction_count"], ascending=[True, False])
            .groupby("terminal_id")
            .head(limit)
        )
